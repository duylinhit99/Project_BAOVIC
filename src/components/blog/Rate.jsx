...
function Rate(props) {
    ...
    API.get(API_URL.RATE + params.id)
      .then((response) => {
        if (response.data && Array.isArray(response.data.data)) {
          let tongRate = response.data.data.reduce(
            (sum, value) => sum + value.rate,
            0
          );
          ...
          const userPreviousRating = response.data.data.find(
  ...
  const changeRating = (newRating, name) => {
    ...
    API.post(API_URL.RATE + params.id, formData, config)
      .then((response) => {
